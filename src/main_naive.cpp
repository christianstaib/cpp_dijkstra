...
std::vector<glm::dvec3> get_gravitational_force(const std::vector<space::CelestialBody> &bodies) {
  ...
  for (size_t i = 0; i < bodies.size(); ++i) {
    for (size_t j = 0; j < bodies.size(); ++j) {
      ...
      force[i] += (bodies[j].mass * distance_vector) / (squared_distance * sqrt(squared_distance));
    }
...
std::vector<space::CelestialBody> read_bodies(std::string path) {
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      space::CelestialBody body = space::CelestialBody::from_state_vactor_string(line);
...
int main() {
  std::vector<space::CelestialBody> bodies = read_bodies("planets_and_moons_state_vectors.csv");
  //
  // read bodies
  //
  ...
  for (int x = 0; x < int(365 / time_step); ++x) {
    ...
    for (int i = 0; i < bodies.size(); ++i) {
      ...
      body.vel += 0.5 * (acc_old + acc_new) * time_step;  // Correctly using both old and new accelerations
    }
